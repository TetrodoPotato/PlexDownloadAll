export class PlexDownload {
     ...
     */
    private getDownloadUrl(pMediaXml: Document, pAccessConfig: AccessConfig): string {
    ...
    private async getAccessConfig(pMediaUrl: string): Promise<AccessConfig> {
        const lClientIdMatch: RegExpExecArray = /server\/([a-f0-9]{40})\//.exec(pMediaUrl);
...
type AccessConfig = { accessToken: string, baseUrl: string; };
type XPathConfig = { accessTokenXpath: string, baseUriXpath: string, partKeyXpath: string; };
type UrlConfig = { apiResourceUrl: string, apiLibraryUrl: string, downloadUrl: string; };
